...
int pl_EtaComparison(){
  ...
  const char* baseName2 = "outputs/wConstraints/purityHists_June9";
  const char* baseName3 = "outputs/noConstraints/purityHists_June9";
  ...
  {
    //with constraints
    sprintf(BEMCName[q],"%s_Eta_%s_BEMC_processed.root",baseName2,trigName[q]);
      cout<<BEMCName[q]<<endl;
    BEMC[q] = new TFile(BEMCName[q],"READ");
    sprintf(SMDLName[q],"%s_Eta_%s_SMD_processed.root",baseName2,trigName[q]);
    SMDL[q] = new TFile(SMDLName[q],"READ");
    sprintf(SMDTName[q],"%s_Eta_%s_SMD2_processed.root",baseName2,trigName[q]);
    SMDT[q] = new TFile(SMDTName[q],"READ");

    // No Constraints
    sprintf(BEMCName[q],"%s_Eta_%s_BEMC_wHFT_processed.root",baseName3,trigName[q]);
    BEMCh[q] = new TFile(BEMCName[q],"READ");
    sprintf(SMDLName[q],"%s_Eta_%s_SMD_wHFT_processed.root",baseName3,trigName[q]);
    SMDLh[q] = new TFile(SMDLName[q],"READ");
    sprintf(SMDTName[q],"%s_Eta_%s_SMD2_wHFT_processed.root",baseName3,trigName[q]);
  ...
  for(int i=0; i<4; i++)
  {
    if(BEMC[i]->IsOpen() && SMDL[i]->IsOpen() && SMDT[i]->IsOpen() && BEMCh[i]->IsOpen() && SMDLh[i]->IsOpen() && SMDTh[i]->IsOpen())
